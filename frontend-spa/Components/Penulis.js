export default {
  template: `
    <div class="p-8">
      <div class="flex justify-between items-center mb-4">
        <h1 class="text-2xl font-bold text-indigo-700">✍️ Penulis</h1>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="(p, i) in list" :key="p.id" class="border-b hover:bg-gray-50">
              ...
              <td class="px-4 py-2">{{ p.nama_penulis }}</td>
              <td class="px-4 py-2">{{ p.bio }}</td>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-xl p-6 w-full max-w-md">
          <h2 class="text-lg font-bold mb-4">{{ form.id ? 'Edit' : 'Tambah' }} Penulis</h2>
          <input v-model="form.nama_penulis" placeholder="Nama Penulis"
  ...
  methods: {
    ...
    openModal(data = null) {
      this.form = data ? { ...data } : { id: null, nama_penulis: '', bio: '' };
      this.showModal = true;
    },
    async simpan() {
      if (this.form.id) {
        await axios.put(`/api/penulis/${this.form.id}`, this.form);
      } else {
        await axios.post('/api/penulis', this.form);
      }
      this.showModal = false;
      await this.load();
    },
    async hapus(id) {
      if (confirm('Yakin hapus penulis ini?')) {
        await axios.delete(`/api/penulis/${id}`);
        await this.load();
      }
    }
  }
